...
/** 明確堅持取消（非首句試探）：才與 INSIST_REFUND 一併觸發 needs_human */
const INSIST_CANCEL_PATTERNS =
  /就是不要了|我還是要取消|堅持(?:要)?取消|不用再問(?:了)?|不用問了|非取消不可|一定要取消|說了要取消|執意取消|別再勸了/i;
...
/** 取消／售後對話中的常見接續句（須搭配最近脈絡才可自人工排隊解鎖） */
export const CANCEL_FLOW_FOLLOWUP_PATTERNS: RegExp[] = [
  /買(?:太)?多了|多買了/i,
  /(?:有點)?後悔/i,
  /預算|太貴|沒錢|不夠/i,
  /等(?:太)?久|不想等/i,
...
export function isCancelFlowFollowup(text: string): boolean {
  const t = (text || "").trim();
  if (!t) return false;
  return CANCEL_FLOW_FOLLOWUP_PATTERNS.some((p) => p.test(t));
}

/** 最近訊息內容是否像在售後／取消／退換貨脈絡（避免無關句誤解鎖） */
export const AFTERSALES_OR_CANCEL_CONTEXT_SNIPPET =
  /取消(?:訂單)?|退貨|換貨|退款|售後|表單|原因|後悔|買多|預算|訂單編號|單號|不要這筆|不要了|想取消|我要取消/i;

export function recentThreadHintsAftersalesOrCancel(recentMessageBodies: string[]): boolean {
  const blob = (recentMessageBodies || []).join("\n");
  return AFTERSALES_OR_CANCEL_CONTEXT_SNIPPET.test(blob);
}

/** 人工排隊中：取消流程接續句 + 最近對話有售後脈絡 → 允許解鎖給 AI */
export function shouldUnlockHandoffForCancelFlowFollowup(
  currentUserText: string,
  recentMessageBodies: string[]
): boolean {
  if (!isCancelFlowFollowup(currentUserText)) return false;
...
function detectReturnReasonType(userMessage: string, primary_intent: PrimaryIntent): ReturnReasonType {
  ...
  if (INSIST_REFUND_PATTERNS.test(t) || INSIST_CANCEL_PATTERNS.test(t)) return "insist";
...
export function resolveConversationState(input: ResolveInput): ConversationState {
  ...
  const insistT = (userMessage || "").trim();
  if (INSIST_REFUND_PATTERNS.test(insistT) || INSIST_CANCEL_PATTERNS.test(insistT)) {
    needs_human = true;
