...
| server/routes.ts | OFF_TOPIC_GUARD_MESSAGE；getProductScopeFromMessage；enforceOutputGuard；off_topic_guard 短路；product_scope 設定/清除；order_lookup／product_scope prompt；output guard 套用於回覆 |
...
| server/phase2-output.ts | OFF_TOPIC_GUARD_MESSAGE、enforceOutputGuard、字數上限常數（與 routes 共用） |
