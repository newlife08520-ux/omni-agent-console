/**
 * MULTI_BRAND_AGENT_OPS_IMPLEMENTATION_BUNDLE_V2.zip
 * - UTF-8 logs, forward slashes via tar -a
 * - staged git diff for listed paths only (then reset)
 */
...
const STAGING_PARENT = path.join(root, "_BUNDLE_V2_STAGING");
const STAGE = path.join(STAGING_PARENT, BUNDLE_NAME);
const OUT_ZIP = path.join(root, "MULTI_BRAND_AGENT_OPS_IMPLEMENTATION_BUNDLE_V2.zip");

/** Paths relative to repo root — Phase 1 / 1.5 backend + docs */
const BUNDLE_PATHS = [
  "package.json",
  "shared/schema.ts",
  "server/db.ts",
...
/** 扁平目錄複製（避免 Windows 上 fs.cpSync 對部分路徑異常結束） */
...
const changedRoot = path.join(STAGE, "changed_source");
for (const rel of BUNDLE_PATHS) {
  copyFile(rel, changedRoot);
}
...
const patchLines = fs.existsSync(path.join(patchesDir, "git_diff.patch"))
  ...
  : 0;
const manifest = {
  bundle: "MULTI_BRAND_AGENT_OPS_IMPLEMENTATION_BUNDLE_V2",
