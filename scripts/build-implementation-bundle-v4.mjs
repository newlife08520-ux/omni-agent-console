/**
 * MULTI_BRAND_AGENT_OPS_IMPLEMENTATION_BUNDLE_V4.zip
 */
...
const STAGING_PARENT = path.join(root, "_BUNDLE_V4_STAGING");
const STAGE = path.join(STAGING_PARENT, BUNDLE_NAME);
const OUT_ZIP = path.join(root, "MULTI_BRAND_AGENT_OPS_IMPLEMENTATION_BUNDLE_V4.zip");
...
const BUNDLE_PATHS = [
  ...
  "scripts/build-implementation-bundle-v4.mjs",
...
function runCapture(cmd, outName, extraEnv = {}) {
  ...
  try {
    ...
    fs.writeFileSync(outPath, msg + code + "\n--- STDERR/ERROR ---\n" + err, "utf8");
    if (outName !== "verify_phase16.txt") throw e;
  }
}
...
const phase16Db = path.join(root, "_evidence_run", "phase16_bundle_verify_db");
fs.mkdirSync(phase16Db, { recursive: true });
runCapture("npm run verify:phase16", "verify_phase16.txt", {
  DATA_DIR: phase16Db,
});
...
const pilotJson = path.join(root, "_evidence_run", "phase16", "pilot_ai_logs_evidence.json");
const pilotZip = path.join(root, "PHASE1_PILOT_FUNCTIONAL_EVIDENCE.zip");

const manifest = {
  bundle: "MULTI_BRAND_AGENT_OPS_IMPLEMENTATION_BUNDLE_V4",
  ...
  environment: {
    bundle_built_on: "local_or_ci_agent",
    phase16_verify_data_dir: phase16Db,
    phase16_data_dir_isolated: true,
  },
  pilot_evidence: {
    pilot_ai_logs_json_exists: fs.existsSync(pilotJson),
    pilot_functional_evidence_zip_exists: fs.existsSync(pilotZip),
    note:
      "若二者為 false，代表建置當下無有效 OPENAI_API_KEY；請本機設定 key 或 .env 後執行 verify:phase16 與 zip:pilot-evidence。",
  },
};
fs.writeFileSync(
