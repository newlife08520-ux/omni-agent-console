...
- **External API**: 一頁商店 (Super Landing) via https://api.super-landing.com — field mapping: recipient→buyer_name, mobile→buyer_phone, email→buyer_email, tracking_codes→tracking_number, created_date→created_at; API limitation: `mobile` param completely ignored (returns all 2.1M orders), sort params also ignored; quad-mode lookup: (1) global_order_id direct, (2) phone-only global date-range scan (today→3d→7d→30d→90d, parallel 5-page batches, client-side phone filtering), (3) product+phone via page_id search with smart date-windowed fallback for high-volume pages, (4) date-range + email/phone/name filter (31-day max); full pages catalog (~3,970 pages = sales page titles) fetched with pagination (133 API pages), hourly cache refresh; AI prompt shows top 100 products, server-side fuzzy matching covers all; system prompt v5.0: phone-only lookup path (no product name required)
