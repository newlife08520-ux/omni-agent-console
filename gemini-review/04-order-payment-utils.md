# 04 - 付款狀態判斷（server/order-payment-utils.ts）

負責判斷每筆訂單的付款狀態：貨到付款 / 已付款 / 付款失敗 / 未付款。
是所有付款顯示的「源頭」。

請審查：
- 貨到付款的判斷是否涵蓋 Shopline 代碼（tw_711_b2c_pay 等）
- 黑貓宅配代收有沒有被正確識別
- 信用卡、LINE PAY 等有沒有正確分類
- fallback 的 PENDING 標籤是否對客友善
