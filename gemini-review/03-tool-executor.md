# 03 - 工具執行器（server/services/tool-executor.service.ts）

AI 呼叫工具後，這個檔案負責：
1. 接收工具參數
2. 查詢訂單
3. 組裝回傳給 LLM 的資料（orderSummaries、formattedList、one_page_full）
4. 決定多筆訂單的回覆格式（≤3 完整卡片、4-5 簡表、6+ 前 5 筆）

請審查：
- 訂單資料有沒有完整回傳（商品名、付款、配送、地址、門市）
- 多筆分層邏輯有沒有正確
- 有沒有內部術語外洩給客人
- 付款狀態判斷是否正確
