# 08 - Shopline 訂單映射（server/shopline.ts）

負責把 Shopline API 回傳的原始資料映射成統一的 OrderInfo 格式。
目前已知問題：商品名稱（name）是空字串。

請審查：
- mapShoplineOrder 的欄位對應是否正確
- 商品名可能存在哪個原始欄位（title? product_title? variant_title?）
- 付款方式（tw_711_b2c_pay 等）有沒有被正確處理
