          ...
          {selectedBrandId != null && !dismissedBotIdHint && (
            <div className="mt-2 p-2 rounded-lg bg-amber-50 border border-amber-200 flex items-start gap-2">
              <p className="text-[10px] text-amber-800 leading-tight flex-1">
                ...
                改完 Bot ID 後，<strong>之前的客人</strong>要等他們「再傳一則新訊息」才會自動歸到此品牌、頭貼／姓名也會補齊；若他們還沒再傳，可先到「全部」找到該客人處理。
                    ...
                    <div className="p-3 space-y-3">
                      ...
                      <Card className="border-stone-200 shadow-sm">
                        ...
                        <CardContent className="px-3 pb-3 pt-0 space-y-1.5 text-[11px]">
                          ...
                          <div className="flex justify-between items-center"><span className="text-stone-500">問題類型</span><Select value={effectiveSelectedContact?.issue_type || "none"} onValueChange={(v) => handleIssueTypeChange(v === "none" ? "" : v)}><SelectTrigger className="w-[100px] h-6 text-[10px] border-stone-200" data-testid="select-issue-type"><SelectValue placeholder="—" /></SelectTrigger><SelectContent><SelectItem value="none">未分類</SelectItem>{(Object.keys(ISSUE_TYPE_LABELS) as IssueType[]).map((it) => <SelectItem key={it} value={it}>{ISSUE_TYPE_LABELS[it]}</SelectItem>)}</SelectContent></Select></div>
                        </CardContent>
                      </Card>
